...
window.onload = function init(){
    ...
    /*function mapToSphere(mouse_x, mouse_y, last_pos, current_pos){  //move this to onmousemove?
        var d = Math.sqrt( Math.pow(mouse_x, 2) + Math.pow(mouse_y, 2));
        if(dragging){
            last_pos = current_pos;
            if (d > (1/Math.sqrt(2))){
                var z = 1 /(2*d);
                current_pos = vec3(mouse_x, mouse_y, z);
            } else {
                z = Math.sqrt(1 - Math.pow(d, 2));
                current_pos = vec3(mouse_x, mouse_y, z);
            }
        }
    }*/
