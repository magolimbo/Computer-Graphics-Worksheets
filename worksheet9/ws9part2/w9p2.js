...
window.onload = function init() {
    ...
    var model    = initObject(gl, filename, 0.15);  //scale to 1/4
    ...
    function rotateLight(){
        if(lightAnimationActive){
            ...
            lightPos[2] = 2.0 * Math.cos(theta) - 2.5;
        }
    }

    function renderShadow(){
        shadow_m[3][1] = -1.0/(lightPos[1]-(-0.5));
        gl.useProgram(gl.program_teapot);
        ...
        var Ms = mat4();
        Ms = mult(Ms, translate(lightPos[0], lightPos[1], lightPos[2]));
        Ms = mult(Ms, shadow_m);
        Ms = mult(Ms, translate(-lightPos[0], -lightPos[1], -lightPos[2]));
        Ms = mult(Ms, M);
        var mLoc = gl.getUniformLocation(gl.program_teapot,'M');
        ...
        gl.uniform1f(gl.getUniformLocation(gl.program_teapot,'isShadow'), 1.0); //set shadow to true
