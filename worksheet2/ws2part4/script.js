...
window.onload = function init(){ 
    ...
    drawCircles.addEventListener("click", clickOnDrawCircles)
    ...
    function clickOnCanvas(ev){
        ...
        {
            if(circle_counter == 0){
                circle_array.push(vec2(normalizedX, normalizedY))
                var size = 0.03; var offset = size/2
                squareVertices = createPointSquare(normalizedX, normalizedY, offset) //crea array di 6 vertici per formare un quadrato
                //update buffer
                gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
                ...
                //pulisco buffer, disegno cerchio
                circle_array.push(vec2(normalizedX, normalizedY))
                ...
                for (var i = 1; i < num_vertices + 2; i++) 
                {
                    ...
                    var temp = vec2(normalizedX - center[0], normalizedY - center[1]);
                    var r = length(temp);
